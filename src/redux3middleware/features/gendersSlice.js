...
const gendersSlice = createSlice({
  ...
  selectors: {
    getUsersByGanders: (state) => {
      return state.filter === 'all'
        ? state.data
        : state.data.filter(user => user.gender === state.filter)
    }
  }
...
export const { getUsersByGanders } = gendersSlice.selectors
export const { changeFilter } = gendersSlice.actions
